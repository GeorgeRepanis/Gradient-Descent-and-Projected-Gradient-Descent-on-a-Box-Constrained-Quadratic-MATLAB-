%% ΘΕΜΑ 4 - Μέθοδος Μέγιστης Καθόδου με Προβολή
clear; clc; close all;
...
% Παράμετροι μεθόδου (Θέμα 4)
s       = 0.1;       % "προ-βήμα" πάνω στο -grad
gamma   = 0.2;       % βήμα προς το προβληθέν σημείο
epsilon = 1e-2;      % ακρίβεια: ||x_k|| <= epsilon
kmax    = 2000;      % μέγιστος αριθμός επαναλήψεων (ασφάλεια)
...
N_hist = norm(xk);   % (προαιρετικά: ιστορικό ||x_k||)
...
ylabel('τιμή f(x_k)');
title('Σύγκλιση της f(x_k) για Μέθοδο Καθόδου με Προβολή (Θέμα 4)');
grid on;
